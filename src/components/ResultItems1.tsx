...
export type ResultItems1Type = {
  className?: string;
  image40?: string;
};

const ResultItems1: FunctionComponent<ResultItems1Type> = ({
  className = "",
  image40,
}) => {
  return (
    <div
      className={`self-stretch shadow-[0px_4px_16px_rgba(17,_34,_17,_0.05)] rounded-xl bg-neutrals flex flex-row items-start justify-start py-[1.5rem] px-[1rem] box-border max-w-full [row-gap:20px] text-left text-[0.75rem] text-blackish-green font-montserrat-medium-14 mq750:flex-wrap ${className}`}
    >
      <img
        className="h-[6.925rem] w-[10rem] relative object-cover"
        loading="lazy"
        alt=""
        src={image40}
      />
      <div className="flex-1 rounded-tl-none rounded-tr-xl rounded-br-xl rounded-bl-none bg-neutrals flex flex-col items-start justify-start py-[0rem] px-[1.5rem] box-border gap-[1rem] min-w-[26.313rem] max-w-full mq750:min-w-full">
        <div className="self-stretch flex flex-row items-center justify-between gap-[1.25rem] mq450:flex-wrap">
          ...
          <div className="flex flex-col items-end justify-start text-gray-400">
            <div className="relative font-medium inline-block min-w-[5.25rem]">
              starting from
            </div>
            <b className="relative text-[1.5rem] inline-block text-slamon text-right min-w-[3.625rem] whitespace-nowrap mq450:text-[1.188rem]">
              $104
            </b>
          </div>
        </div>
